...
export default async function PostDetailPage({params}: PostPageProps) {
  ...
  // ── 產生文章 URL ──────────────────────────────────────────
  const canonicalUrl = `https://lasttiom-official-website.vercel.app/blog/${params.slug}`

  // ── 封面圖 URL（有就用，沒有就不帶 image 欄位）───────────
  const ogImageUrl = post.mainImage?.asset
    ? urlForImage(post.mainImage).width(1200).height(630).url()
    : null

  // ── Article Schema ────────────────────────────────────────
  // 讓 Google 知道：這是誰寫的、什麼時候發布、什麼時候更新
  const articleSchema = {
    ...
    author: {
      '@type': 'Person',
      name: '止時雙寶媽',
      url: 'https://lasttiom-official-website.vercel.app/about',
    },
    publisher: {
      '@type': 'Organization',
      name: '止時 LAST·TIME',
      url: 'https://lasttiom-official-website.vercel.app',
      logo: {
        '@type': 'ImageObject',
        // 換成你實際的 logo 路徑
        url: 'https://lasttiom-official-website.vercel.app/logo.png',
      },
    },
    mainEntityOfPage: {
      '@type': 'WebPage',
  ...
  // ── FAQ Schema ────────────────────────────────────────────
  // 讀取 Sanity 的 faq 欄位（陣列，每筆有 question / answer）
  // 如果你的 Sanity schema 裡還沒有這個欄位，下面會安全地跳過
